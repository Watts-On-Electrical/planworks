...
function JobCard({ j, cdef, P, dark, onClick }) {
  ...
  return (
    <div onClick={onClick} style={{ cursor: "pointer", background: P.surface, border: "1px solid " + P.line, borderLeft: "4px solid " + c.color, borderRadius: 6, padding: "7px 8px 8px", marginBottom: 6, opacity: cancelled ? 0.6 : 1 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 6 }}>
        <span style={{ fontFamily: COND, fontWeight: 700, fontSize: 14, color: P.accent2 }}>{j.t}</span>
        <Pill k={j.st} dark={dark} />
      </div>
      <div style={{ fontFamily: SEMI, fontWeight: 700, fontSize: 13.5, color: cancelled ? P.muted : P.ink, lineHeight: 1.15, textDecoration: cancelled ? "line-through" : "none" }}>{j.site}</div>
...
export default function WorkPlanner() {
  ...
  const openNew = (c, di) => setEditing({ id: uid(), w: week, c, d: di, t: "08:00", site: "", type: "Rewire", addr: "", cust: "", st: "booked", notes: "", isNew: true });
  ...
  const openDuplicate = () => setDuplicating({ base: editing, contractors: [editing.c], days: [editing.d] });
  ...
  const runDuplicate = async () => {
    ...
    const copies = [];
    d.contractors.forEach((c) => d.days.forEach((day) => {
      if (c === b.c && day === b.d) return; // skip the original cell
      copies.push({ id: uid(), c, w: b.w, d: day, t: b.t, site: b.site, type: b.type, addr: b.addr, cust: b.cust, st: b.st, notes: b.notes });
    }));
  ...
  const exportImage = async () => {
    ...
    } catch (e) { setExporting(false); alert("Could not create image: " + (e && e.message ? e.message : e)); }
  };
  ...
  return (
    <div style={{ minHeight: "100vh", padding: "32px 30px 44px", display: "flex", flexDirection: "column", alignItems: "center", gap: 14, fontFamily: FONTS, background: P.page }}>
      ...
      <div ref={cardRef} style={{ width: 1300, maxWidth: "100%", background: P.surface, border: "1px solid " + P.line, borderRadius: 14, overflow: "hidden", boxShadow: dark ? "0 16px 44px -22px rgba(0,0,0,.6)" : "0 16px 44px -22px rgba(20,40,50,.35)" }}>
        ...
        <div style={{ display: "flex", alignItems: "stretch", background: P.colHead }}>
          <div style={{ width: 186, flex: "none", padding: "12px 16px", color: P.headerSub, fontFamily: COND, fontWeight: 600, fontSize: 13, letterSpacing: ".09em", textTransform: "uppercase", borderRight: "2px solid rgba(255,255,255,.22)" }}>Contractor</div>
          {days.map((d, i) => (
        ...
        {roster.map((c) => (
          <div key={c.id} style={{ display: "flex", alignItems: "stretch", borderTop: "1px solid " + P.rowSep }}>
            <div style={{ width: 186, flex: "none", padding: "11px 14px", background: dark ? P.surfaceAlt : (c.color + "14"), borderRight: "2px solid " + P.line, display: "flex", alignItems: "center", gap: 9 }}>
        ...
        <div onClick={(ev) => { if (ev.target === ev.currentTarget) setDuplicating(null); }} style={{ position: "fixed", inset: 0, background: "rgba(10,20,25,.55)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, zIndex: 60 }}>
          <div style={{ width: 460, maxWidth: "100%", maxHeight: "92vh", overflow: "auto", background: P.surface, borderRadius: 14, boxShadow: "0 24px 60px -20px rgba(0,0,0,.55)" }}>
            ...
            <div style={{ padding: "18px 20px", display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ fontSize: 12.5, color: P.muted }}>Creates a separate, independent copy in each selected person + day (the original stays as-is). Copies land in the week you're viewing.</div>
