// ============================================================================
// BOQ TEMPLATE â€” "Electrical Bill of Quantities".
...
// Reconcile a saved BOQ against the CURRENT drawing (all sheets): re-count every
// mapped line, (re)build the auto-added lines for symbols not on the template, and
// keep the user's rates and custom lines. Idempotent - safe to run on every open.
export function reconcileBoq(boq, project, SYMBOL_META, findSymbol) {
  const counts = countSymbols(placedOf(project));
  const sfq = secondFixQtys(counts);
  const mapped = new Set(Object.values(SECONDFIX_SYMBOLS).flat());

  const freshAdditional = Object.entries(counts)
    .filter(([id]) => !mapped.has(id))
    .map(([id, qty]) => {
      const m = SYMBOL_META[id] || {};
      const sym = findSymbol ? findSymbol(id) : null;
      return {
        symbolId: id,
        item: m.description || (sym && sym.name) || id,
        spec: m.height && m.height !== "\u2014" ? m.height : "",
        qty, rate: 0, sf: null, fromDrawing: true,
      };
    })
  ...
  const sections = boq.sections.map(sec => {
    ...
    const kept = sec.items
      .filter(it => !it.fromDrawing)
      .map(it => it.sf ? { ...it, qty: (sfq[it.sf] != null ? sfq[it.sf] : "") } : it);
    const oldDrawing = sec.items.filter(it => it.fromDrawing);
    const rebuilt = freshAdditional.map(fresh => {
      const prev = oldDrawing.find(o =>
        (fresh.symbolId && o.symbolId === fresh.symbolId) || o.item === fresh.item);
      return prev ? { ...fresh, id: prev.id, rate: prev.rate } : { ...fresh, id: rid() };
    });
    return { ...sec, items: [...kept, ...rebuilt] };
  });
  return { ...boq, sections };
}
