 ...
 * supabase/company-logos-setup.sql), not just here -- a partial unique index
 * for the company row, and slots 0/1 for the accreditations.
 *
...
export async function saveCompanyLogo({ kind, dataUrl, slot = 0 }) {
  ...
  const { data, error } = await client()
    .from("company_logos")
    .upsert(row, {
      onConflict: kind === KIND_COMPANY ? "user_id" : "user_id,sort_order",
